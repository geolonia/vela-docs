...
| `user` | 標準 API アクセス | 割り当てられたテナントとServicePath内のデータ操作 |
...
GeonicDB は **XACML 3.0** ポリシーを使用した、きめ細かい属性ベースのアクセス制御をサポートしています。ポリシーは、サブジェクト属性（ユーザーロール、テナント）、リソース属性（エンティティタイプ、ServicePath）、アクション属性（read、write、delete）に基づいてルールを定義します。
