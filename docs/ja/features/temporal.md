...
Temporal API は NGSI-LD の時間的表現モデルに準拠しています。時系列トラッキングが有効な場合、GeonicDB は各エンティティ属性の更新を時間インデックス付きのレコードとして保存し、属性の完全な履歴をクエリできるようにします。
...
GeonicDB はストレージの増大を管理するために、時系列データの TTL 設定をサポートしています。設定された TTL を超えた時系列レコードは自動的にパージされます。
